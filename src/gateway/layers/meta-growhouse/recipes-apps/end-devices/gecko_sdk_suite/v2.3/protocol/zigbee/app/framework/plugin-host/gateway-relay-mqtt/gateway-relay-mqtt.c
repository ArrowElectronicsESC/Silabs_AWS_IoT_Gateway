...
static void publishMqttHeartbeat(void)
{
  ...
  char* panIdString;
  EmberEUI64 myEui64;
  char myEui64Str[32] = {0};

  /* get gateway zigbee coordinator eui64 */
  emberAfGetEui64(myEui64);
  snprintf(myEui64Str, 32, "%.2X%.2X%.2X%.2X%.2X%.2X%.2X%.2X", myEui64[7], myEui64[6], myEui64[5], myEui64[4], myEui64[3], myEui64[2], myEui64[1], myEui64[0]);
  ...
  if (!emberAfNcpNeedsReset() && status == EMBER_SUCCESS) {
    ...
    free(panIdString);
    cJSON_AddStringToObject(heartbeatJson, "eui64", myEui64Str);
                                  ...
                                  uint8_t payloadStartIndex)
{
  ...
  /* Memory allocated to this json object is getting overwrite in the function.
     so comment the line to avoid memory leak */
...
void emberAfPluginGatewayRelayMqttInitCallback(void)
{
  ...
  // Create ota-device-list object
  otaDeviceList = cJSON_CreateArray();
}
...
bool emberAfPreMessageReceivedCallback(EmberAfIncomingMessage* incomingMessage)
{
	...
	{
		if (incomingMessage->message[2] == 0x20)
		{
			...
			if (packet_cur - packet_pre > 1) {
				...
				/* If packet number 255 is missed */
				emberAfCorePrintln("Test Result: PER %d% , Total Packet lost : %d",
...
#endif
	/* Log message added to check signal strengh of received packet	*/
	emberAfDeviceTableGetEui64FromNodeId(incomingMessage->source, gatewayEui64);
	/*
	emberAfCorePrintln(" Received Signal Strength rssi == %d  of device %02X%02X%02X%02X%02X%02X%02X%02X",incomingMessage->lastHopRssi,
			gatewayEui64[7],
			gatewayEui64[6],
			gatewayEui64[5],
			gatewayEui64[4],
...
/** @brief Function to add device eui64String in ota-device-list
 *
 * @param eui64 : deviceUid of zigbee end-device
 */
void addOtaDevice(EmberEUI64 eui64) {

  char eui64String[EUI64_STRING_LENGTH] = {0};
  ...
  sprintf(eui64String,"%02X%02X%02X%02X%02X%02X%02X%02X", eui64[7], eui64[6], eui64[5], eui64[4] , eui64[3], eui64[2], eui64[1], eui64[0]);

  /* First check if device is already added into the list*/
  arraySize = cJSON_GetArraySize(otaDeviceList);
  for ( count = 0 ; count < arraySize; count++ ) {
    deviceList = cJSON_GetArrayItem(otaDeviceList, count);
    objectString = cJSON_GetObjectItem(deviceList, "eui64")->valuestring;
    if (!strncmp(eui64String, objectString, strlen(eui64String))) {
      return;
    }
  }

  objectJson = cJSON_CreateObject();
  cJSON_AddStringToObject(objectJson, "eui64", eui64String);
  cJSON_AddItemToArray(otaDeviceList, objectJson);

  char * ptr = cJSON_PrintUnformatted(otaDeviceList);
  emberAfAppPrintln ("Allow OTA for Device = %s\n",ptr);
  free(ptr);
}

/** @brief Function to remove device eui64String in ota-device-list
 *
 * @param eui64 : deviceUid of zigbee end-device
 */
void removeOtaDevice(EmberEUI64 eui64) {
  ...
  arraySize = cJSON_GetArraySize(otaDeviceList);
  for ( count = 0 ; count < arraySize; count++ ) {
    objectJson = cJSON_GetArrayItem(otaDeviceList, count);
    objectString = cJSON_GetObjectItem(objectJson, "eui64")->valuestring;
    if (!strncmp(eui64String, objectString, strlen(eui64String))) {
      cJSON_DeleteItemFromArray(otaDeviceList, count);
      break;
    }
  }
...
/** @brief Function to print ota-device-list
 *
 * @param eui64 : deviceUid of zigbee end-device
 */
void printOtaDevice() {
...
/** @brief Function to check if device has requested for OTA.
 *
 * @param eui64 : deviceUid of zigbee end-device
 */
bool isDeviceOtaRequested(EmberEUI64 eui64){
  cJSON * objectJson = NULL;
  char eui64String[EUI64_STRING_LENGTH] = {0};
  char *objectString = NULL;
  int arraySize = 0;
