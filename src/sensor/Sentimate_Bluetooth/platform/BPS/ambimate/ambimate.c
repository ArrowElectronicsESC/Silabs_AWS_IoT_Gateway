...
uint8_t ambimate_read_all(void)
{
	...
	if (ret == (uint8_t)i2cTransferDone)
	{
		...
		am_dev.light 		= (am_rx_buff[5] * 256.0 + am_rx_buff[6]);
		am_dev.audio 		= (am_rx_buff[7] * 256.0 + am_rx_buff[8]);
		am_dev.battV 		= ((am_rx_buff[9] * 256.0 + am_rx_buff[10]) / 1024.0) * (3.3 / 0.330);
		am_dev.co2_ppm		= (am_rx_buff[11] * 256.0 + am_rx_buff[12]);
		am_dev.voc_ppm		= (am_rx_buff[13] * 256.0 + am_rx_buff[14]);
	}
	/* Read Data */
