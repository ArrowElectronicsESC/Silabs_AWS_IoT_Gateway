...
// Note: change this to set the desired duty cycle (used to update CCVB value)
 LED_RGB_TypeDef_t BoardLED = {PWM_LED_START, PWM_LED_START, PWM_LED_START};
 ...
 * @brief  Rainbow effect on LED RGB
 *****************************************************************************/
void app_LED_feedback(uint8_t state, uint32_t event_cnt)
{
	...
	case 1: /* APP_IDLE Led Blink every 2 Seconds */
	case 2: /* APP_NOT_CONNECTED */
		if ((event_cnt % 2) == 0)
		{
			BoardLED.duty_led_r = 0;
			BoardLED.duty_led_g = 25;
			BoardLED.duty_led_b = 0;
		}
		else
		{
			BoardLED.duty_led_r = 0;
			BoardLED.duty_led_g = 0;
			BoardLED.duty_led_b = 0;
		}
		change_PWM_LED_colors();
		break;

	case 3: /* APP_CONNECTED */
		if ((event_cnt % 2) == 0)
		{
			BoardLED.duty_led_r = 0;
			BoardLED.duty_led_g = 0;
			BoardLED.duty_led_b = 25;
		}
		else
		{
			BoardLED.duty_led_r = 0;
	...
	case 4:	/* APP_SEND */
		BoardLED.duty_led_r = 25;
		BoardLED.duty_led_g = 0;
		BoardLED.duty_led_b = 0;
		change_PWM_LED_colors();
		break;

	case 5:	/* APP_AUDIO_BEAT */
		BoardLED.duty_led_r = 25;
		BoardLED.duty_led_g = 25;
		BoardLED.duty_led_b = 0;
		change_PWM_LED_colors();
